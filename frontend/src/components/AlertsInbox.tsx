...
export default function AlertsInbox() {
  ...
  const fetchAlerts = () => {
    fetch(`\${process.env.NEXT_PUBLIC_API_URL || `\${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}`}/api/v1/alerts`, { credentials: "include" })
      .then(res => res.json())
      .then(data => {
  ...
  const handleResolve = async (id: string) => {
    ...
    try {
      await fetch(`\${process.env.NEXT_PUBLIC_API_URL || `\${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}`}/api/v1/alerts/${id}/resolve`, {
        method: "PATCH",
        credentials: "include"
      });
    } catch (err) {
